...
class data_analysis:
    ...
    def normalize_series(self,data_series):
        ...
        #### FILL IN CODE HERE ####
        s = sum(data_series)
        normalized_series = list(map(lambda x: x/s, data_series))
    ...
    def compute_average_across_series(self,list_of_series):
        ...
        #### FILL IN CODE HERE ####
        '''
        Find the length of the first list in list_of_series to initiate a sum list being that all lists are the same length
        Iterate through each i of list_of_series and add to new_list by index
        Divide each element of new_list by the length of list_of_series to get average_list
        '''
        # sum up all corresponding elements of series
        x = len(list_of_series[0])
        new_list = [0 for i in range(x)]
        for i in list_of_series:
            new_list = list(map(lambda x,y: x+y, i, new_list))
        c = len(list_of_series)
        average_list = list(map(lambda x: x/c, new_list))
    ...
    def compute_standard_deviation_across_series(self,list_of_series,average_series):
        ...
        #### FILL IN CODE HERE ####
        '''
        Form new_list of size len(list_of_series[0])
        Iterate through each i of list_of_series
            subtract each average_series element from each i element and add to new_list of same index
        Divide each element of new_list by len(list_of_series)
        Square each element of resultant average_list
        '''
        x = len(list_of_series[0])
        new_list = [0 for i in range(x)]
        for i in list_of_series:
            new_list = list(map(lambda x,y,z: ((x-y)**2)+z, i, average_series, new_list))
        c = len(list_of_series)
        average_list = list(map(lambda x: x/c, new_list))
        std_list = list(map(lambda x: x**.5, average_list))
    ...
    def compute_correlations(self,data_series_1,data_series_2):
        ...
        #### FILL IN CODE HERE ####
        l = len(data_series_1)
        avg1 = sum(data_series_1)/l
        avg2 = sum(data_series_2)/l
        correlation_list = list(map(lambda x,y: (x-avg1)*(y-avg2), data_series_1, data_series_2))
    ...
    def compute_net_correlation(self,data_series_1,data_series_2):
        ...
        net_correlation = sum(correlation_list)
    ...
    def compute_ratio(self,data_series_1,data_series_2):
        ...
        #### FILL IN CODE HERE ####
        l = len(data_series_1)
        ratio_list = [0 for i in range(l)]
        for i in data_series_2:
            if i == 0:
                print('Error, division by zero detected, exiting program....')
                exit()
    ...
    def compute_standard_deviation_of_series(self,data_series,average_of_series):
        ...
        '''
        #### FILL IN CODE HERE ####
        new_series = list(map(lambda x: (x-average_of_series)**2, data_series))
        
        series_standard_deviation = (sum(new_series)/len(data_series))**.5
    ...
    def question2(self):
        ...
        for i in range(len(countries_raw)):
            if gdppc_raw[index] > gdp_per_capita_threshold:
                high_net_birth_rate_countries.append(countries_raw[index])
                high_net_birth_rate.append(gdppc_raw[index])
            else:
                low_net_birth_rate_countries.append(countries_raw[index])
                low_net_birth_rate.append(gdppc_raw[index])
            index += 1
    ...
    def question3(self,energy_consumption_average_IN,ratio_kids_survive_over_kids_fatality_IN):
        ...
        '''
        #### FILL IN CODE #### START
        t1 = list(map(lambda x: (.12-.01)/10*x+.01, [i for i in range(10)]))
        #### FILL IN CODE #### END
        ...
        '''
        #### FILL IN CODE #### START
        t2 = list(map(lambda x: (270-30)/10*x+30, [i for i in range(10)]))
        #### FILL IN CODE #### END
